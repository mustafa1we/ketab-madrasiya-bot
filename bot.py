...
كن لطيف وطبيعي ومختصر.

مهم جداً:
افهم كلام الزبون حتى لو بي أخطاء إملائية أو لهجة عراقية.

لا تطلب من الزبون إعادة السؤال إذا كان واضح.

لا تستخدم عبارة:
"وضحلي شنو تحتاج 🌷"
إلا إذا السؤال فعلاً غير مفهوم.

إذا سأل الزبون عن منتج:
جاوبه حسب الأسعار والمعلومات الموجودة.

لا تخترع سعر.

إذا ما عندك سعر مؤكد:
قل:
السعر يحتاج تأكيد من المكتبة 🌷

إذا ما تعرف توفر المنتج:
قل:
...
def direct_reply(user_text):
    ...
    if (
        ...
        return (
            "إي عدنا دفاتر هواي 🌷\n"
            "منها دفاتر سجل، دفاتر ملاحظات، "
            "دفاتر رسم وتلوين وغيرها.\n"
            "إذا تريد أگلك الأنواع والأسعار."
        )
...
def find_product(user_text):
    ...
    # Exact product name inside message
...
يوجد أكثر من سعر لهذا المنتج:
{prices_text}

لا تختار سعراً من نفسك.
إذا احتجت، اطلب تحديد النوع أو الحجم.
"""


    return """
ماكو منتج محدد مطابق.

إذا السؤال واضح، جاوب بصورة طبيعية.
...
def process_message(msg):
    ...
    if "text" in msg:
        ...
        try:
            ...
            # إذا الرد معروف، لا نستخدم DeepSeek
            ...
            # =================================================
            # إذا ماكو رد مباشر، نستخدم DeepSeek
            # =================================================
